...
void Scheduler::ReadyToRun(Thread *thread)
{
    ...
    {
        if (L1ReadyQueue->IsEmpty() && (L2ReadyQueue->IsEmpty() || priority > L2ReadyQueue->Front()->getPriority()))
        {
            Run(thread, false);
        }
        else
...
static int l1cmp(Thread *x, Thread *y)
{
    return x->getPredictedBurstTime() - y->getPredictedBurstTime();
}
...
void Scheduler::UpdatePriority()
{
    ...
    for (ListIterator<Thread *> *it3(L3ReadyQueue); !it3->IsDone(); it3->Next())
    {
        Thread *thread = it3->Item();
        thread->setWaitTime(thread->getWaitTime() + TimerTicks);
        if (thread->getWaitTime() > 900)
        {
    ...
    for (ListIterator<Thread *> *it2(L2ReadyQueue); !it2->IsDone(); it2->Next())
    {
    ...
    for (ListIterator<Thread *> *it1(L1ReadyQueue); !it1->IsDone(); it1->Next())
    {
