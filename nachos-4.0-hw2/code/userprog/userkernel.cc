...
int UserProgKernel::Exec(char *name, int priority)
{
    //<TODO
    ...
    //TODO>
    Thread *thread = new Thread(name, threadNum);
    thread->setPriority(priority);
    thread->Fork((VoidFunctionPtr)&ForkExecute, (void *)thread);

    threadNum++;
    return threadNum - 1;
}

void UserProgKernel::ExecAll()
{
    //<TRACE
